...
{
    //Considerar que o automóvel faz 12 Km/L
    int TempoGasto = int.Parse(Console.ReadLine());
    int VelocidadeMedia = int.Parse(Console.ReadLine());

    double CombustivelNecessario = (TempoGasto * VelocidadeMedia) / 12.0;

    Console.WriteLine();
    Console.WriteLine(CombustivelNecessario.ToString("F3", CultureInfo.InvariantCulture));
    Console.ReadLine();
}
